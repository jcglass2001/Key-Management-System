...
public class Main extends Application {
	...
	private Scene buildAdd() {
		...
		// place building and room entries to the left
		root.add(addScene_Left(), 0, 0);
		// place suite and employee entries to the right
		root.add(addScene_Right(), 1, 0);
	...
	private Pane buildBuildingEntry_Add() {
		...
		vBoxBuildingInfo.getChildren().addAll(hBoxBuildingNameInfo, hBoxBuildingIdInfo);
	...
	private Pane buildSuiteEntry_Add() {
		...
		vBoxSuiteInfo.getChildren().addAll(hBoxSuiteNameInfo, hBoxSuiteIdInfo, hBoxBuildingIdInfo);
		// return pane
	...
	private Pane buildRoomEntry_Add() {
		...
		vBoxRoomInfo.getChildren().addAll(hBoxRoomNumInfo, hBoxSuiteIdInfo, hBoxBuildingIdInfo);
	...
	private Pane buildEmployeeEntry_Add() {
		...
		vBoxEmpInfo.getChildren().addAll(hBoxFirstNameInfo, hBoxMiddleInitInfo, hboxLastNameInfo, hBoxEmpIdInfo);
		// return pane
		return vBoxEmpInfo;
	}
	
	/* builds building and room entry container for the add scene */
	private Pane addScene_Left() {
		VBox vbox = new VBox();
		vbox.getStyleClass().add("h_or_v_box");
		vbox.getChildren().addAll(buildBuildingEntry_Add(), buildRoomEntry_Add());
		return vbox;
	}

	/* builds suite and employee entry container for the add scene */
	private Pane addScene_Right() {
		VBox vbox = new VBox();
		vbox.getStyleClass().add("h_or_v_box");
		vbox.getChildren().addAll(buildSuiteEntry_Add(), buildEmployeeEntry_Add());
		return vbox;
	}
